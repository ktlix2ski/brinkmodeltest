...
# Surface elevation
class Surface(df.UserExpression):
  def eval(self,values,x):
    values[0] = (zmax - zmin)*np.exp(-(x[0]+L)/(L*0.3)) + zmin - amp*(np.sin(4*np.pi*x[0]/L)) + thklim

# Bed elevation
class Bed(df.UserExpression):
  def eval(self,values,x):
    values[0] = (zmax - zmin)*np.exp(-(x[0]+L)/(L*0.3)) + zmin - amp*(np.sin(4*np.pi*x[0]/L))
...
for f in df.facets(mesh):
    if df.near(f.midpoint().x(),L):
       ocean[f] = 1  # Oceanic boundary
    if df.near(f.midpoint().x(),-L):
       ocean[f] = 2  # Ice divide
