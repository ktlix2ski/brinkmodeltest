...
###### Noise Parameters ######
test = 'RNG(7)_amax_matchAmin'
rand = np.random.default_rng(7)
beta = 2
t_end = 3000

# amin_noise = cn.powerlaw_psd_gaussian(beta, t_end, random_state=rand)
amax_noise = cn.powerlaw_psd_gaussian(beta, t_end, random_state=rand)

noise_amplitude = 1.0 # m/yr
...
amin_base = df.Constant(-16.0) 
amax = df.Constant(8.0)         # Maximum smb
amax_base = df.Constant(8.0)        
...
nx = 900
mesh = df.IntervalMesh(nx,0,2*L, name="TWG_mesh")       #OG uses length of domain as 2*L
...
ph_SMB, = ax_disp[1].plot(SMB.dat.data[:], surf1.dat.data[:], 'b-')
...
frames_disp = []
def update_anim_disp(frame):
    t = time_dict[frame]
    time_text2.set_text(f'Time: {t:.2f} years')

    ph_bed1.set_ydata(bed_dict[t])
    ph_base1.set_ydata(base_dict[t])
    ph_surf1.set_ydata(surf_dict[t])
    ph_sed1.set_ydata(sed_dict[t])

    ph_SMB.set_ydata(surf_dict[t])
    ph_SMB.set_xdata(SMB_dict[t])

    xmin = min(SMB_dict[t])
    xmax = max(SMB_dict[t])
    ax_disp[1].set_xlim(xmax + 2, xmin - 2)
    ...
    return[time_text2, ph_bed1, ph_base1, ph_surf1, ph_sed1, ph_SMB]
...
success_filename_disp = os.path.join(full_anim,f"FullSMB_{start_time}.mp4")
...
interrupt_filename_disp = os.path.join(anim_rupt,f"InterruptedSMB_{start_time}.mp4")
...
def interrupt(signal, frame):
    ...
    ani_rupt2 = animation.FuncAnimation(fig_disp, update_anim_disp, frames = len(frames_disp), interval=100, blit=False)
    ani_rupt2.save(interrupt_filename_disp, writer="ffmpeg", fps=10, dpi=150)  # Save the crashed animation
...
while t<t_end:
    try: # If the solvers don't converge, reduce the time step and try again.
        ...
        current_year = int(t // 1.0)

        if current_year > 1 and current_year != previous_year:
            previous_year = current_year

            # Apply noise for this year
            # amin_new = float(amin_base) + amin_noise[current_year]
            amax_new = float(amax_base) + amax_noise[current_year]

            # amin.assign(amin_new)
            amax.assign(amax_new)

            print(f"amin={amin}")
            print(f"amax={amax}")
            noise_list.append([current_year,float(amin),float(amax)])
...
#KT: adding these in case of intterupt 
    ...
    if (dt_float < 1e-10): # Things have really gone off the rails and the simulation is shutting down
        ...
        np.savetxt(f"amin_amax_timeseries_amp{noise_amplitude}_{t_end}yrs_{test}.csv", noise_arr, delimiter=",", header="time,amin,amax", comments='', fmt="%.6f")
        ...
        ani_rupt2 = animation.FuncAnimation(fig_disp, update_anim_disp, frames = len(frames_disp), interval=100, blit=False)
        ani_rupt2.save(interrupt_filename_disp, writer="ffmpeg", fps=10, dpi=150)  
...
if t >= t_end-5:
    ...
    print("Finished saving dictionary and csv, onto state variables...")
    ...
    print("Now onto animation saving... ")
    ...
    ani2 = animation.FuncAnimation(fig_disp, update_anim_disp, frames = len(frames_disp), interval=100, blit=False)
    ani2.save(success_filename_disp, writer="ffmpeg", fps = 10, dpi = 150)
